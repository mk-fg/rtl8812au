...
)
{
	...
	if ((strcmp(input[1], "ccap") == 0)) {
		...
		phydm_set_crystal_cap(p_dm, (u8)arg);
		PHYDM_SNPRINTF((output + used, out_len - used, "X_cap = ((0x%x))\n", p_cfo_track->crystal_cap));

	} else if ((strcmp(input[1], "test-rssi-mod") == 0)) {
		PHYDM_SSCANF(input[2], DCMD_DECIMAL, &arg);
		p_dm->test_rssi_offset = (s8)arg;
		PHYDM_SNPRINTF((output + used, out_len - used, "p_dm->test_rssi_offset = %d\n", p_dm->test_rssi_offset));

	} else if ((strcmp(input[1], "test-noisy-off") == 0)) {
		PHYDM_SSCANF(input[2], DCMD_DECIMAL, &arg);
		p_dm->test_noisy_disable = (boolean)arg;
		PHYDM_SNPRINTF((output + used, out_len - used, "p_dm->test_noisy_disable = %d\n", p_dm->test_noisy_disable));

	} else {
		PHYDM_SNPRINTF((output + used, out_len - used, "%s\n", "==========================="));
		PHYDM_SNPRINTF((output + used, out_len - used, "%s\n", "Available tune_para tweaks:"));
		PHYDM_SNPRINTF((output + used, out_len - used, "%s\n", "==========================="));
		PHYDM_SNPRINTF((output + used, out_len - used, "ccap {dec | -1 hex} : crystal_cap = 0x%x\n", p_cfo_track->crystal_cap));
		PHYDM_SNPRINTF((output + used, out_len - used, "test-rssi-mod: phydm->test_rssi_offset = %d\n", p_dm->test_rssi_offset));
		PHYDM_SNPRINTF((output + used, out_len - used, "test-noisy-off: phydm->test_noisy_disable = %d\n", p_dm->test_noisy_disable));
	}
